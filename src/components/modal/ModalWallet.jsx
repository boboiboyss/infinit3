...
const ModalWallet = ({ isOpen, onClose, openWalletConnected, setSelectedWallet }) => {
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
      <div className="relative rounded-lg shadow-lg w-full sm:w-3/4 md:w-2/3 lg:w-1/2 bg-[#C5D2F7] h-[480px] sm:h-auto sm:max-h-[80vh] overflow-y-auto">
        <div className="flex flex-col md:flex-row h-full">
          <div className="px-4 border-b md:border-b-0 md:border-r border-black w-full md:w-1/3">
            <img src={logo} alt="logo" className="h-16 mb-4" />
            <h2 className="text-xl font-bold mb-4 text-center md:text-left">Connect your wallet</h2>
            ...
            <div className='flex items-center justify-center md:justify-start'>
              <div className='flex items-center justify-center space-x-2'>
                <p className='text-lg mt-auto'>I don't have a wallet</p>
          ...
          <div className="w-full md:w-2/3 p-4 md:p-6">
            ...
            <div className="grid grid-cols-2 gap-4 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-2">
              {wallets.map((wallet, index) => (
                <button key={index} onClick={() => handleWalletClick({ id: wallet.id, name: wallet.name, image: wallet.image })}>
                  <div className="p-4 border border-black rounded-xl bg-white flex items-center space-x-4 hover:shadow-lg transition">
                    <img src={wallet.image} alt={wallet.name} className="w-12 h-12 object-cover rounded-full" />
                    <span className='font-semibold text-justify'>{wallet.name}</span>
                  </div>
                </button>
              ))}
            </div>
